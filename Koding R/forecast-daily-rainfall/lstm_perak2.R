...
library(ggplot2)
ggplot(data.frame(data), aes(x = 1:nrow(data), y = `X1`)) + geom_line()
ggplot(data.frame(data[1:100,]), aes(x = 1:100, y = `X1`)) + geom_line()
...
val_steps <- (200 - 101 - lookback) / batch_size

# How many steps to draw from test_gen in order to see the entire test set
test_steps <- (nrow(data) - 201 - lookback) / batch_size
